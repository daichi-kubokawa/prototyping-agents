...
```
仕様生成 10〜13分     設計で短縮できる。実際に2回短縮した
人手の操作 30秒       削る余地がほとんどない
Figma Make 5分45秒〜15分17秒（5回計測）  外部サービスの都合。制御できない
```

Figma Make は**同一のプロンプトでも7分と15分17秒**を記録している。生成量ではなく
サービス側の変動が支配変数であり、**生成範囲を削っても短くなる保証はない。**
仕様の質だけが失われるため、範囲は変更しない。
