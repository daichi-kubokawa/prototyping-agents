...
**改善は2回行っている。それぞれ同一の入力で前後を計測した。**

### ① 制約の置き場所を直した

| | 改善前 | 改善後 | |
|---|---:|---:|---|
| 仕様生成の所要時間 | 21分50秒 | **11分31秒** | ▲47% |
...
| | 改善前 | 改善後 | |
|---|---:|---:|---|
| 仕様生成の所要時間 | 17分09秒 | **11分50秒** | ▲31% |
| サブエージェント委譲回数 | 6回 | **2回** | ▲67% |
| 画面数 | 3画面 | **4画面** | ＋1 |
| `content.md` | 不要判定 | **生成** | ＋1成果物 |

成果物ごとにエージェントを呼び直しており、**1回あたり1〜3分のコールドスタート**が積み上がっていた。
「一括生成してはならない」は**順序**の規約であって**委譲回数**の規約ではない、と読み替え、
1回の委譲の中で逐次生成させた。**成果物が増えたうえで5分19秒短縮している。**

> ①と②の間でフレームワーク自体が育っている（送信安全性の検査、回収ディレクトリの自動生成、
> 実文言の表形式化）。**11分31秒と17分09秒は地続きではない。**

### 計測の範囲（重要）

**上記はいずれも「仕様生成」の時間である。** 計測しているのは、
入力を渡してから `figma_make_prompt.md` が出るまでで、**画面に到達するまでではない**。

商談で実際にかかる時間は、これに次が加わる。

| # | 工程 | 時間 | 精度 |
|---|---|---:|---|
| ① | 仕様生成（入力 → `figma_make_prompt.md`） | **11分50秒** | 実測 |
| ② | ファイル5点の添付とプロンプト貼付（人手） | **30秒** | 実測 |
| ③ | Figma Make の生成 | 7分〜15分17秒 | **分散あり（n=2）** |
| | **画面到達まで** | **約20〜28分** | ①②は実測、③は幅で記録 |

### ③の分散について

**同一のプロンプトで2回計測し、7分と15分17秒だった。** 入力が変わっていないため、
生成量ではなく **Figma Make 側の変動**が支配変数である。したがって
**DRAFT の生成範囲を削っても、③が短くなる保証はない。**
仕様の質と ITERATE の前提だけが失われるため、スコープは変更しない。

なお7分のほうは計測精度が低い（完了時刻を正確に取れていない）。
**確度が高いのは 15分17秒 のほう**であり、n=2 では平均を出す意味がない。範囲で記録する。

### 自分の設計が支配している範囲

```
①＋②  12分20秒        本フレームワークが決めている。実測
③      7〜15分         Figma Make の生成。こちらでは制御できない
```

**改善の対象は①だけである。** ②は人手だが30秒しかなく、削る余地がない。
③は外部サービスの都合であり、短縮を主張してはならない。

> 商談で待ち時間が問題になる場合、フレームワークを削るのではなく
> **その場で Figma Make に渡す画面数を絞る**（まず1画面、合意してから追加する）。
> これは実行時の判断であって、仕様の範囲を狭めることではない。

### 現在値

```
仕様生成    11分50秒（実測） / 4画面 / 466行 / 委譲2回 / 検査 全8件 PASS
人手の操作   30秒（実測）
画面到達まで  約20〜28分（うち Figma Make 7〜15分・分散あり）
```
...
継続案件の場合は、**前回の Open Questions を一覧表示**する ―― これが商談前に叩く最大の実用価値になる。
...
> **2回目以降の商談は「継続モード」で走る。**
> 既存の成果物があれば、前回を読んで**差分だけを更新**する。全部は作り直さない。
> 変わっていない箇所を再生成すると、**前回お客様と合意した表現が理由なく変わる**ためである。
> 継続モードでは人間に問い返さない（商談中に止まらない）。
...
商談が終わったら、Figma Make の最終出力を `<案件>/figma_output/` に保存する。

**`/log` は伝票であって実物ではない。** Figma Make は依頼以上のことをしたり、
異なる解釈をしたりする。実物が無いと、以下を検出できない。

- **記録漏れ** ―― ログに無い変更が画面に入っている
- **仕様との乖離** ―― `Guidelines.md` のトークンと、実際に使われた値が違う

コードで回収するのが望ましい。トークン値を機械的に突き合わせられるため、
スクリーンショットより検出精度が高い。
...
**本フレームワークの範囲は、確定した仕様と動くプロトタイプまでである。** 本実装は含まない。

商談で見せた画面は Figma Make が生成している。それを捨てて別実装を起こすと、
お客様が合意した画面と実装物が別々に育つ。整合を取る仕組みが要り、速度の利点が消える。

実装可能な単位への分解（`tasks.md` ―― Bolt と DoD 4観点）までを引き渡し資料として作り、
そこから先は受け取る側の開発プロセスに委ねる。**線を引かないことは、責任範囲を曖昧にすることである。**
...
**出口**：`FAIL` の全件と、**仕様を確定として引き渡してよいか**の判定
