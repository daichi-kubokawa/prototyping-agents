...
商談1件を、準備からプロトタイプの引き渡しまで通したときの流れ。

---

## 実測値

公開している2案件の実測。**リポジトリの成果物と対応している。**

|                              | `helpdesk-app` |  `night-app` |
| ---------------------------- | -------------: | -----------: |
| 仕様生成（`/draft`）         |   **10分51秒** | **12分55秒** |
| 添付とプロンプト貼付（人手） |           30秒 |         30秒 |
| Figma Make の生成            |       11分34秒 |      5分45秒 |
| **画面に到達するまで**       |   **22分55秒** | **19分10秒** |
| 配色                         |  Light（既定） | **Dark**（導出） |
| 画面数                       |              2 |            3 |

**同じ仕組み、正反対の配色。** 入力した利用状況だけが違う。
ただし `night-app` の Dark は利用状況から導出した結果、`helpdesk-app` の Light は
装置と物理環境が聞けていないための既定値であり、`design.md` にそう明記している。

### 何が制御できて、何ができないか

```
仕様生成    10〜13分              設計で短縮できる
人手の操作  30秒                  ほぼ固定
Figma Make  5分45秒〜15分17秒     外部サービス側の変動
```
...
改善は2回。それぞれ同一の入力で前後を計測した。
**下表は仕組みを改善したときの計測であり、公開している2案件の数字ではない**（計測用の実行は破棄した）。
...
flowchart TD
    ...
    subgraph M["商談当日 ―― 止まらない"]
        ...
        D["<b>④</b> 顧客反復<br/>Figma Make ⇄ /log<br/>17分"]
    end
...
gantt
    ...
    顧客反復（見せる・直す）      :active, h2, 00:38, 17m
    まとめ                      :h3, 00:55, 5m

    section AI が主導
    /draft 仕様生成 実測11分       :crit, a1, 00:15, 11m
    Figma Make 生成 6〜12分       :crit, a2, 00:26, 12m
```

**ここで直視すべきは、顧客反復に使える時間が17分しか残らないことである。**
`/draft` が11分、Figma Make の初回生成が12分。合計23分が生成に費やされる。
Figma Make が遅い日は、これが13分まで縮む。

Figma Make の生成時間は外部サービスの都合であり、こちらでは短縮できない（5回の計測で5分45秒〜15分17秒）。
したがって**反復の回数を確保する手段は、生成そのものを軽くすることだけ**である。

> **初回は1画面だけ生成させる。** 仕様は全画面ぶん持ったまま、Figma Make に渡す範囲を絞る。
...
23分を沈黙で過ごせば商談は終わる。ここは仕組みではなく、人間側の準備で決まる。
...
長時間の録音や既存資料は、事前に整理してから入力する。
ただし利用状況や原文のニュアンスが失われないよう、要約しすぎない。
