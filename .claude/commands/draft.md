...
| S-5 | **アップロード安全性** | アップロードする5点に、生の発言引用・`briefing.md` 参照・実URL・資格情報が残っていないか。**スクリプトで機械的に判定する** |

**S-5 はスクリプトを実行して判定する。目視しない。**

```
python3 scripts/check_upload_safety.py <OUTPUT_DIR>
```

検出された場合、**保留にせずその場で直す。** 由来の記述を `requirements.md` の識別子
（`M-3` `C-2` 等）へ置換し、再実行して 0 件を確認する。数十秒で終わる。

> **これだけは「保留」にできない。** 保留のまま Figma Make へ上げれば、
> お客様の発言がその瞬間に外部サービスへ渡る。**取り消せない。**
> 止まらないことと、送ってしまうことは別である。
