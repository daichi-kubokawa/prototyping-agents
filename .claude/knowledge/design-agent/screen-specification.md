...
**このファイルは Figma Make へアップロードする。**
そのため、**画面に出る文字列以外を書いてはならない。** 根拠も引用も注記も入れない。
...
| ID | 画面 | 種別 | 表示文字列 | 由来 |
|---|---|---|---|---|
| T-01 | S02 | 見出し | 見積・納期の妥当性を確認する | M-3 |
| T-02 | S02 | 本文 | 参照した基準と、判断した理由を残しておく | M-3 |
| T-03 | S01 | 空状態 | 進行中の案件はありません | M-1 |
````
...
| 列 | 中身 |
|---|---|
| `ID` | `T-nn`。`screens.md` の表示要素から参照できるようにする |
| `画面` | `screens.md` の画面ID（`S01` 等） |
| `種別` | 見出し / 本文 / ラベル / ボタン / **選択肢** / 空状態 / エラー |
| `表示文字列` | **画面にそのまま出る文字列。** 説明でも要約でもない |
| `由来` | **`requirements.md` の識別子のみ**（`M-3` `C-2` 等）。文章を書かない |

`表示文字列` 列だけを抜き出せば、実物との照合がそのまま機械的に行える。

### 記述規約

- **書くのは「実際に画面に出る文字列」だけ**である。仕様の説明文は書かない
- **選択肢の値も表示文字列である。**「メール／口頭／その他を選択」と散文で書くのではなく、
  1行ずつ `選択肢` として登録する。**登録されていない文字列は、寸法の設計対象から漏れる**
- **`briefing.md` の発言を直接引用してはならない**（`guardrails.md` §2）。
  由来は識別子で示す。生の発言は外部サービスへ送るべきものではない
- 根拠を持たない内容を創作しない。**業務知識を勝手に発明すると、お客様の前で「それ違います」と言われる**
- 根拠が薄い項目は行ごと作らず、`requirements.md` の Open Questions に送る
