...
## 3. ワークフローは二周する（DRAFT / ITERATE）＋ 実装（BUILD）
...
**実装（BUILD・`BD-n`）は、この2周の外側にある。**
DRAFT と ITERATE は同じ成果物を2周するが、BUILD は**その成果物を消費して
別のもの（`index.html`）を作る**。性質が違うためコマンドを分けている。
...
> 本ファイルでは重複させない。
>
> なお ADR の識別子は `D-01` `D-02`（2桁）であり、ステップ記号 `DR-n` / `IT-n` / `BD-n` とは別物である。
