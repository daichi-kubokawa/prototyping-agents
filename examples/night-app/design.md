...
- **`recordedAt` は日付を含む完全なタイムスタンプとする（時刻のみを禁止する）**。シフトは17時〜翌9時（M-3）と日付をまたぐため、時刻のみの表示では深夜0時前後に確定した記録の前後関係を一意に判別できず、M-22 の自己防衛証跡としての機能を損なう。表示形式（日付の省略表記を含むか等）は content.md の確定時に定めるが、内部表現・保存値には常に日付を含めること（IT-2 開放的レビュー由来、review_history.md 参照）
